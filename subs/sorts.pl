...
sub sort_records{
	...
	if ($record->{record_nature} ne '' && $record->{record_nature}!~/nonprinting/i && $output ne 'conlist'){      # && $output ne 'conlist'  , no they need not be in the sort!
		$stop=1;
	...
	if ($record->{doc_type} eq 'Review' && $output ne 'revlist' && $output ne 'rlg' && $stop==0){
		...
		unless ($output eq 'conlist'){
       	...
       	###if running make_MODS include only things that were already printed
       	if  ($output eq 'make_MODS'){
       		$stop=1;
       		
       	}
       	if  ($output eq 'make_MODS' && $record->{date_pub_print} ne ''){
       		if ($record->{record_nature}!~ /duplicate/i && $record->{record_nature}!~/scope/i && $record->{record_nature}!~/skip/i && $record->{record_nature}!~/nonprinting/i){
       		 	
       		#the sort order does not matter so just mark in the hash so later those records can be picked up
       		unless ( $record->{date_pub_print} =~/PRINT/){
       			 	
       			unless ($record->{doc_type} =~/review/i && $record->{description}=~/essay/i){ 
       			
       				#this is temporary for a record with bad data
       				unless($record->{record_number} eq '930814'){
       					$sort_array{ $record->{record_number} } = 1;
       				}
       			}	
       		 }
       		}
       		#set this so that will not waist time creating a sort sequence below
       	...
       	$sort='';
       	#also make a second array for sorting of reviews, but don't do this if runnig make_MODS
